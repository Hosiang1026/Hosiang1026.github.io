...
###### 2.3、MySQL多实例主键自增 
这个方案用于解决MySQL的单点问题，在auto_increment基础上，设置step步长![Test](https://oscimg.oschina.net/oscnet/up-70ee7123f5f2e3e06ce53252f6d19c21c4f.png  '分布式ID生成方案') 
...
在分布式场景中，服务器时钟回拨会经常遇到，一般存在10ms之间的回拨。虽然10ms看似很短，但此算法是建立在毫秒级别的生成方案，一旦回拨，就很有可能存在重复ID。 
...
##### 一线大厂的设计思路其实和普通开发者的思路类似，只是在设计上多考虑了1～2层，增加了1～2个环节。 
...
让我们看看如何优化这个方案。首先分析数据库压力大的原因：每次获取ID时，都需要向数据库请求一次。那么能否避免每次都去数据库获取呢？ 
...
双buffer的方案，小伙伴们有没有感觉很酷，这样就达到了业务场景用的ID，都是在jvm内存中获得的，从此不需要到数据库中取。允许数据库宕机时间更长了。 
因为会有一个线程，会观察什么时候去自动获取。两个buffer之间自行切换使用。就解决了突发阻塞的问题。 
 
##### 四、总结 
 
##### 此方案是某团使用的分布式ID算法，如果想了解更多细节，可以参考相关技术文档，本文已介绍了核心实现思路。 
