...
提到分布式算法，就不得不提 Paxos 算法，在过去几十年里，它基本上是分布式共识的代名词，因为当前最常用的一批共识算法都是基于它改进的。
...
```
  `后面我们会了解这些消息的含义。 代码还引入了一些常量，其中大多数常量定义了各种消息的超时：```java
```
      JOIN_RETRANSMIT = 0.7
...
```
  `最后我们需要定义协议中的Proposal和Ballot```java
```
