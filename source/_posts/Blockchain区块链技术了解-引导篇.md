...
什么是比特币？与我们学习的区块链技术之间有什么关系呢？这些不法分子为什么指定要比特币，而不接受时下流行的微信或支付宝转账或者让你带上一包现金送到某个天桥下面呢？这款病毒是不是经营比特币的公司为了让大家购买比特币而制造的？
...
比特币（英文为Bitcoins，简写为BTC）是一种用开源P2P软件生成的新型电子虚拟货币。最初由日本程序员中本聪（Satoshi Nakamoto）于2009年提出，其在开源社区Sourceforge.net搭建一个Bitcoin项目，结合开源软件和密码学中的块密码工作模式，在P2P对等网络和分部式数据库的平台上，开发出比特币发行和交易、账户管理的操作系统，让遍布整个对等网络用户端的各节点按照其种子文件达成网络协议。
