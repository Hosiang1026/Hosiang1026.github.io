...
#### 1.1 常见认证规范/协议 
 ...
 OAuth2 是一种协议规范，定义了几种用来身份验证和权限授权的处理方式。它是一种可扩展的协议规范，涵盖了几种复杂的使用场景。并且包含了基于第三方身份验证的处理方法。我们常见的"使用微信登录"、"使用 QQ 登录"等第三方登录方式的底层技术就是基于 OAuth2 实现的。 
 1.2 
 ...
 OpenIDConnect 是另一种基于 OAuth2 的协议规范。它扩展了 OAuth2 的部分功能，让以前相对模糊的功能变得可操作性更强。常见的 Google 登录就是基于 OpenID Connect 实现的。 
 1.3 
...
#### 1.2 
 ...
 头部（Header） 
```
 JWT 的头部承载两部分信息：1、声明类型，这里是 jwt2、声明加密的算法，通常直接使用 HMAC SHA256我们使用 base64 解析一下 eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9 这个子串 可以得到: `{` ```json
```
"typ": "JWT",
...
```
 我们使用 base64 解析一下 eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lIiwiYWRtaW4iOnRydWV9 就可以得到 之前定义的一个 payload: `{` ```json
```
"sub": "1234567890",
...
``` `scheme_name: str = None,` ```java
          scopes: dict = None,
  ``` `auto_error: bool = True,` ```java
      ):
  ```
`if not scopes:` `scopes = {}` ```java
...
```
  ```
`if self.auto_error:` `raise HTTPException(` ```java
                      status_code=HTTP_401_UNAUTHORIZED,
  ``` `detail="Not authenticated",` ```java
```
                      headers={"WWW-Authenticate": "Bearer"},
```
  ``` `)` `else:` `return None` ```java
          return param
  ```
  
  
 为了完成我们接下来的功能，需要大家进行两个模块的安装: 
...
`pip install python-multipart` 
 ...
 在生产实践过程中，获取 token 的代码： `from datetime import datetime, timedelta` ```python
```python
from typing import Optional
```
  
```
`` `from fastapi import Depends, FastAPI` ```python
...
```
  ```
`if expires_delta:` `expire = datetime.utcnow() + expires_delta` `else:`
...
```
  ...
  ```
      return encoded_jwt
  ```
`# 请求接口` `@app.post("/token", response_model=Token)` ```java
...
#### 1.5 
 ...
 只愿能帮助那些需要这些内容的同行或刚入行的小伙伴，我希望能在推广 python 技术的道路上尽我一份力量，欢迎在评论区向我提问，我都会一一解答。 
