...
上面这段表示是一个字段引用，同样引用了 ```text 
 ...
 ```text 
  Utf8
  ``` ：我们经常使用的是字符编码，但是这个不是只有字符编码的意思，它表示一种字符编码是 ```text 
  Utf8
...
这里为什么都是1呢？这不是默认的构造方法吗？哪来的参数？其实Java语言有一个潜规则：在任何实例方法里面都可以通过 ```text 
  this
  ``` 来访问到此方法所属的对象。而这种机制的实现就是通过Java编译器在编译的时候作为入参传入到方法中了，熟悉 ```text 
  python
  ``` 语言的同学肯定会知道，在 ```text 
  python
  ``` 中定义一个方法总会传入一个 ```text 
  self
  ``` 的参数,这也是传入此实例的引用到方法内部，Java只是把这种机制后推到编译阶段完成而已。所以，这里的1都是指 ```text 
  this
  ``` 这个参数而已。 
 ```xml 
           0: aload_0
