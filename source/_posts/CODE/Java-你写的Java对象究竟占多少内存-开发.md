...
#### 概述 
Java 作为一个面向对象语言，给我们带来了多态，继承，封装等特性，使得我们可以利用这些特性很轻松的就能构建出易于扩展，易于维护的代码。作为一个 ```text 
  Javaer
  ``` ，天天搞“对象”，那你写的对象究竟占用了多少内存呢？我们来看看你的“对象”是如何“败家”的。 
 
#### Java 对象头内存模型 
我们先来看看，一个Java 对象的内存模型是怎么样的？由于我们的虚拟机是分为32位和64位，那肯定它们的模型也是有区别的，下面我列出列32位虚拟机和64位虚拟机下的 ```text 
  Java
  ``` 对象头内存模型。  
...
因为笔者的本地环境是 ```text 
  jdk1.8
  ``` ,64位虚拟机，这里我以64位虚拟机（开启指针压缩）来分析，因为默认情况下， ```text 
  jdk1.8
  ```  在64位虚拟机默认开启指针压缩。 
Java 对象头主要包括两部分，第一部分就是  ```text 
  Mark Word
  ``` ，这也是  ```text 
  Java
  ```  锁实现原理中重要的一环，另外一部分是  ```text 
  Klass Word
  ``` 。 
Klass Word 这里其实是虚拟机设计的一个 ```text 
  oop-klass model
  ``` 模型，这里的 ```text 
  OOP
  ``` 是指 ```text 
  Ordinary Object Pointer
  ``` （普通对象指针），看起来像个指针实际上是藏在指针里的对象。而  ```text 
  klass
  ```  则包含 元数据和方法信息，用来描述  ```text 
  Java
  ```  类。它在64位虚拟机开启压缩指针的环境下占用 32bits 空间。 
Mark Word 是我们分析的重点，这里也会设计到锁的相关知识。 ```text 
  Mark Word
  ```  在64位虚拟机环境下占用 64bits 空间。整个 ```text 
  Mark Word
  ``` 的分配有几种情况： 
 ...
 可偏向（Biased）： 线程id 占54bits， ```text 
  epoch
  ```  占2 bits，分代年龄（ ```text 
  age
  ``` ）占用4 bits，偏向模式（biased_lock）占用1 bits，锁标记（lock）占用2 bits，剩余 1bit 未使用。 
 轻量锁定（Lightweight Locked）： 锁指针占用62bits，锁标记（ ```text 
  lock
  ``` ）占用2 bits。 
 重量级锁定（Heavyweight Locked）：锁指针占用62bits，锁标记（ ```text 
  lock
  ``` ）占用2 bits。 
...
所以，没有进行内存对齐就会导致CPU进行额外的读取操作，并且需要额外的计算。如果做了内存对齐，CPU可以直接从地址0开始读取，一次就读取到想要的数据，不需要进行额外读取操作和运算操作，节省了运行时间。我们用了空间换时间，这就是什么我们需要内存对齐。 
...
我们知道了一个空对象是占用16字节，那么一个非空对象究竟占用多少字节呢？我们还是写一个普通类来验证下： 
 ```java 
  ...
  ```  
这个演示类中引入了别的对象，我们知道 ```text 
  int
  ``` 类型是占用4个字节, ```text 
  NullObject
  ``` 对象占用16字节，对象头占12字节，还有一个很重要的情况  ```text 
  NullObject
...
我们可以看到 ```text 
  TestNotNull
  ``` 的类占用空间是24字节，其中头部占用12字节，变量 ```text 
  a
  ``` 是 ```text 
  int
  ``` 类型，占用4字节,变量 ```text 
  nullObject
  ``` 是引用，占用了4字节，最后填充了4个字节，总共是24个字节，与我们之前的预测一致。但是，因为我们实例化了 ```text 
  NullObject
