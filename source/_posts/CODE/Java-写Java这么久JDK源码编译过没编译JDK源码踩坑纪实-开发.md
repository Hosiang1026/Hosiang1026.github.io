...
![Test](https://codesheep.oss-cn-hangzhou.aliyuncs.com/blog/20200715232858.png  '写Java这么久，JDK源码编译过没-编译JDK源码踩坑纪实') 
![Test](https://codesheep.oss-cn-hangzhou.aliyuncs.com/blog/20200715232858.png  '写Java这么久，JDK源码编译过没-编译JDK源码踩坑纪实') 
原因也很简单，因为实际支撑调试运行的代码，并不是我们解压出来的那份 ```text 
  JDK
  ``` 源码，那个仅仅是做关联用，实际运行用到的 ```text 
  JDK
  ``` ，还是之前系统安装好的那个 ```text 
  JDK
  ``` 环境。 
要想解决这个问题，那就只能使用自己修改过的代码来自行编译生成自己的 ```text 
  JDK
  ``` ，然后用到项目中去！ 
所以什么都憋说了，肝就完了！ 
...
这一点在 ```text 
  ...
  ``` 操作系统和 ```text 
  macOS
  ``` 操作系统上已经天然的保证了，而对于 ```text 
  Windows
  ``` 兄弟来说稍微麻烦一点，需要通过使用 ```text 
  Cygwin
  ``` 或者 ```text 
  MinGW/MSYS
  ``` 这种软件来模拟。 
...
比如我在配置 ```text 
  ...
  ``` 的问题： 
![Test](https://codesheep.oss-cn-hangzhou.aliyuncs.com/blog/20200715232858.png  '写Java这么久，JDK源码编译过没-编译JDK源码踩坑纪实') 
明明系统里已经有编译器，但还是报这个错误。通过后来修改  ```text 
...
make all
  ...
  ```  
这一步编译需要一点时间，耐心等待一下即可。编译过程如果有错误，会终止编译，如果能看到如下两个画面，那么则恭喜你，自己编译 ```text 
  JDK
...
从两张图的对比可以看出，编译 ```text 
  ...
  ``` 和 ```text 
  JDK 11
  ``` 完成时在输出上还是有区别的。时间上的区别很大程度上来源于 ```text 
  JDK 11
  ``` 的编译机配置要高不少。 
...
进入 ```text 
  j2sdk-image
  ``` 目录会发现，里面的内容和我们平时从网络上下载的成品 ```text 
  JDK
  ``` 内容一致。 
![Test](https://codesheep.oss-cn-hangzhou.aliyuncs.com/blog/20200715232858.png  '写Java这么久，JDK源码编译过没-编译JDK源码踩坑纪实') 
...
![Test](https://codesheep.oss-cn-hangzhou.aliyuncs.com/blog/20200715232858.png  '写Java这么久，JDK源码编译过没-编译JDK源码踩坑纪实') 
为了使我们新加的代码行生效，我们必须要重新去JDK源码的根目录中再次执行  ```bash
make images
  ``` 重新编译生成JDK方可生效： 
![Test](https://codesheep.oss-cn-hangzhou.aliyuncs.com/blog/20200715232858.png  '写Java这么久，JDK源码编译过没-编译JDK源码踩坑纪实') 
因为之前已经全量编译过了，所以再次 ```bash
make ``` 的时候增量编译一般很快。 
重新编译之后，我们再次运行 ```text 
  JdkTest
  ``` 项目，就可以看到改动的效果了： 
...
原因很简单，因为我们虽然做了源代码目录的映射，但是实际支撑运行的 ```text 
  JDK
  ``` 还是预先安装好的那个JDK环境，并不是根据我们修改后的源码来重新编译构建的，所以看到这里，解决这个问题就很简单，就像上面一样自行编译一下 ```text 
  JDK
  ``` 即可。 
...
实不相瞒，就这个问题排查了一段时间，熬到了很晚。最终折腾了一番，通过如下这种方式解决了顺便分享给小伙伴们，大家如果遇到了这个问题，可以参考着解决一下。 
因为从控制台的报错可以很明显的看出，肯定是字符编码相关的问题导致的，而且都指向了 ```text 
  ascii
  ``` 这种编码方式。 
于是将JDK的源码从根目录导入了Vs Code，然后全目录查找 ```text 
  encoding ascii
