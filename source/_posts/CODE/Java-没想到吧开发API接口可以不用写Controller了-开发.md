...
其实只要人手够时间够，这些都没问题，但问题就在于立项到上线周期就给十几而开发时间满打满算不到10来天，时间紧又不给加人，底层程序员的生活真是太难了。 
...
##### 实践
直接访问 
 `127.0.0.1:9999/magic/Web` 
 打开 
 可视化界面，看到如下的界面。 
![Test](https://img-blog.csdnimg.cn/20210402132106177.png  '没想到吧，Java开发 API接口可以不用写 Controller了') 创建一个分组，其中 
 `分组前缀` 
```
 为一组API接口的访问根目录，相当于 `@Controller("/order")` 注解。 ![Test](https://img-blog.csdnimg.cn/20210402132106177.png  '没想到吧，Java开发 API接口可以不用写 Controller了') 接着在分组中创建接口  
```
 `order_detail` 
 ，页面配置接口的基础信息，接口名称、请求路径、请求方法、请求参数、请求header等，接口直接 
 `return` 
```
 返回内容 `return ‘小富最帅’` ![Test](https://img-blog.csdnimg.cn/20210402132106177.png  '没想到吧，Java开发 API接口可以不用写 Controller了') 
```
在页面访问刚刚创建接口的全路径  
 `127.0.0.1:9999/order/order_detail` 
  ，发现已经成功返回数据。 
![Test](https://img-blog.csdnimg.cn/20210402132106177.png  '没想到吧，Java开发 API接口可以不用写 Controller了') 也可以直接拼 
 `JSON` 
 格式数据直接返回 
```
如果URL传参 `/order_detail/{id}` ，导入 
```
 `request` 
 模块获取参数 
...
list.each(it => sum+= it + 1)
  ...
  `这里我只简单的介绍了使用，还有很多高级特性，比如：调用```text
  Java API
  `、集成```text
  redis
  `、```text
  Mongo
  `等，感兴趣的同学自己看下官方文档吧，它还提供了很多语法```text
  demo
  ``` 
 ，拿来即用就好。 
地址： 
