...
Java 18 正式版新特性：简易 HTTP 服务器、UTF-8 默认编码、简单 Web Server 等。
...
可以指定的参数包括：
 ...
 `--bind-address addr`
```
 ：指定绑定地址，默认 addr 是： `127.0.0.1 or ::1 (loopback)` `-d dir`
```
  or
 `--directory dir`
...
如果你想改回原来那种根据操作系统环境指定默认字符集的方式，可以使用这个启动参数： `-Dfile.encoding=COMPAT` #### 通过方法句柄（MethodHandle）重新实现 Java 反射接口
