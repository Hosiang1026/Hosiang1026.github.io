...
```
Java 18将正式发布， 虽然它不是长期支持 (LTS) 版本，但它却实现了九个 JEP（在Java 18列出）。有哪些特性值得关注呢？今天胖哥为你提前解读。 JEP 40...
```
<!-- more -->

                                                                                                                     
```
明天Java 18将正式发布， 虽然它不是长期支持 (LTS) 版本，但它却实现了九个 JEP（在Java 18列出）。有哪些特性值得关注呢？今天胖哥为你提前解读。 
```
##### JEP 400
