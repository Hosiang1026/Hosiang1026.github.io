...
```
  ...
  `大于等于运算符:```java
  >=
  `只有当左操作数大于或者等于右操作数时才返回 true。```java
```javascript
  console.log(5 >= 1);   // true
...
```
  ...
  `小于等于运算符:```java
  < 
  `只有当左操作数小于或者等于右操作数时才返回 true。```java
```javascript
  console.log(5 <= 1);   // false
console.log(5 <= 5);   // true
console.log(5 <= 10);  // true
...
```javascript
赋值运算符： `=` 赋值运算符向变量赋值。 `var x = 7;` 加法赋值运算符： `+=` 赋值运算符向变量添加值。 
```
