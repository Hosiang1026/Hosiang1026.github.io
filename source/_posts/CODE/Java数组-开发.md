...
``` 在我们了解数组这个概念之前，我们先思考下面几个问题。如果我们需要两个数据，那么直接创建两个变量即可int a;int b；如果需要五个数据，那么可以创建五个变量int a;int b;int c;int d;int f；但如果我们需要100个甚至是1万个数据，那么我们创一万个变量
...
我们看一个例子，如何获取数组的长度？ 
```
如果你学过C语言，那么肯定会说 `int sz = sizeof(arr) / sizeof(arr[0]);` 其实Java比c语言更方便，直接使用“数组名。length”就可以求出来数组的长度。 
```
代码如下： 
...
for循环示例 
  ...
  `我们发现打印出来了数组的元素。for-each示例```java
        for(int x:arr){
            System.out.print(x+" ");
        }
  ``` 
...
```
  ...
  `范围拷贝使用Arrays.copyOfRange(数组名，从开始下标，到结束下标）```java
```
        int[] arr1 = {1,2,3,4,5,6};
...
int main()
{
  ...
  `而Java中也存在这样的函数：Arrays.sort```java
        int[] arr = {5,4,3,2,1};
        Arrays.sort(arr);
