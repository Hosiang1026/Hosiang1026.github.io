...
##### 构造限流器
```
我们首先构造一个限流器对象： `limiter := NewLimiter(10, 1);` 这里有两个参数： 
```
...
AllowN 方法表示，截止到某一时刻，目前桶中数目否少为 n 个，满足则返回 true，同时从桶中消费 n 个 token。 反之返回不消费 Token，false。 
