...
我是在 Ubuntu 18.04 下编译的，不是在 Windows，建议不在 Windows 下折腾，会遇到比在 Linux 环境下多得多的问题。如果你电脑也是 Windows，可以像我这样，安装个 VMware 虚拟机软件，在里面装个 Ubuntu 系统，在虚拟机里面玩，这 2 个软件在官网下载就行，当然如果懒得去找也可以在我的博客获取 VMware 软件和 Ubuntu 18.04 镜像。 
...
"13-internal" 2019-09-17
  ```  字样，我们编译出来的 JDK13 可以用啦。 ![Test](https://api.opics.org/api  '全网最新最简单的 OpenJDK13 代码编译') 
这标题起得不过分吧，上面版本日期是 2019-09-17，还不到一个月，网上相关资料也没，就只有官方文档了（当然也是最好的资料）。 ```text 
  跟着步骤走，不会丢
  ``` ，为了验证这句话，还把上面的步骤在我快退休的笔记本上跑了一遍，妥妥的。 
#### 回顾 
我们这篇文章就讲了编译 OpenJDK13，接下来会再写一篇怎么搭建 Debug JVM 环境，可以 ```text
推荐阅读
  ```  
