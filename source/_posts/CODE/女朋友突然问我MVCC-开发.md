...
前言 都知道事务的可重复读级别实现原理是使用MVCC实现的，那么你对MVCC的底层实现原理知道多少呢？面试高频点，你值得拥有。 一、MVCC到底是什么？ MVCC即多版本控制器，其特点就是在同一时。..
<!-- more -->

                                                                                                                                                                                        ### 前言 
都知道务的可重复读级别实现原理是使用MVCC实现的，那么你对MVCC的底层实现原理知道多少呢？面试高频点，你值得拥有。 
### 一、MVCC到底是什么
...
在事务并发的情况下会产生以下问题。 
 ...
 幻读：一个事务读取了某个围数据，同时另一个事务新增了这个范围的数据，再次读取发现俩次得到的结果不一致。 
...
#### 3. 如何区分当前读、快照读
```sql
不加锁的简单的select都属于快照读。 `SELECT id name user where id = 1;` 与之对应的则是当前读，给select加上共享锁、排它锁。 
```
...
假设一开始的数据为下图 
```sql
此时执行了一条更新的SQL语句 `UPDATE user set name = 'niuniu where id = 1'` ,那么undo log的记录就会发生变化 
```
...
那么首先会根据事务101去版本链对比，事务101和事务100都会落在 `min_id<=trx_id<=max_id` 这个区间，并且还都在数组中，所以数据是不可见的。 
```
那么继续往版本链中寻找就会遇到事务102这个是最大的事务ID并且不在数组中，所以是可见的。 
于是最终的返回结果还是 
 。 
#### 案例四
