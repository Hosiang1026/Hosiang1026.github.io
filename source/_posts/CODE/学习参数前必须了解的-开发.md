...
![Test](https://api.opics.org/api  '学习JVM参数前必须了解的') 
#### JVM参数通常设置的位置 
...
其中 ```text 
  ...
  ``` 和 ```text 
  -
  ``` 开头的通常会被转换为一个或者多个 ```text 
  -XX:
  ``` 开头的参数，只是一个简化的写法，比如说 ```text 
  -Xmx100M
  ``` ，JVM里会自动转化为 ```java 
  -XX:MaxHeapSize=100M
  ``` ， ```text 
...
我们可以在启动参数里设置一个参数就好，这个参数类似如下 
 ```java 
  ...
  ```  
设置过这个参数之后，我们只要在服务的/home/admin目录下创建flags文件，同时在这个文件里指定所有的JVM参数就可以了，但是对flags文件里的参数写法会有些要求，-X之类的参数不能设置，但是可以用其等价的-XX的参数来替代，比如说-Xmx100M，只能用-XX:MaxHeapSize=100M来取代，同时在文件里不要出现 ```text 
  -XX:
  ``` ，只要 ```java 
  key=value
  ``` 或许 ```text 
