...
### 一、摘要
 ...
 和 
 `Hashtable` 
 了？ `HashMap` 它又线程不安全在哪里？ `ConcurrentHashMap` 的进化与骚操作有哪些？ 
...
#### 2.1 一、为什么Vector和Hashtable被留在了历史的长河中？
  很简单，别想太多，就是因为性能不好。 
 ...
 方法： 
     ...
     ![Test](https://api.opics.org/api  '并发容器与并发控制 - JUC') 
...
#### 2.3 三、ConcurrentHashMap 1.7与1.8的差别是什么？
...
```
    查询复杂度 当冲突超过8个时也能保证查询效率，由原来的拉链法的 `O(n)` 变为红黑树的 `O(logn)` #### 四、ConcurrentHashMap 如何实现如a++这样的组合操作？ 
```
  显然， `ConcurrentHashMap` 是保证了同时put的并发安全性，但是并没有说，你同时执行 
 `a++` 
 时也是线程安全的： 
...
复制代码
  ...
  把这段用 
 包起来吗？ 
  那使用 `ConcurrentHashMap` 还有什么意义呢？我们来看看这样一个方法：replace 
...
复制代码
  `再看一个组合操作，用于解决相同key的put覆盖问题：```text
  putIfAbsent
  ```
  // 核心思想如下表示
if (!map.contains("key")){
...
#### 2.4 五、CopyOnWriteArrayList适用场景有哪些？
...
```
 ...
 ，其实就相当于  
```
  + `Object.wait()` + `Object.notify()` ，这是一个适用于生产者消费者的绝佳队列，如果想看原生实现可以瞅瞅我在并发原理中利用 wait + notify 实现的生产者消费者。 
```
 ...
 `SynchronousQueue` 
```
 : 容量为0不是1、线程池 `Executors.newCacheThreadPool()` 使用的阻塞队列 
```
     
      
       
       
     它没有实际的容量，任意线程（生者程或者消费者线程，生产类型的操作比如put，offer，消费类型的操作比如poll，take）都会等待知道获得数据或者交付完成数据才会返回，一个生产者线程的使命是将线程附着着的数据交付给一个消费者线程，而一个消费者线程则是等待一个生产者线程的数据。 
...
#### 3.1 七、CountDownLatch有哪些典型的适用场景？
...
#### 3.3 九、CyclicBarrier与CountDownLatch的区别是什么？
