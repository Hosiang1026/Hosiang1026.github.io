...
本文 Github开源项目：github.com/hansonwang99/JavaCollection 中已收录，有详细自学编程学习路线、面试题和面经、编程资料及系列技术文章等，资源持续更新中... 工���人 曾几何时，对于Jav...
<!-- more -->
...
### 工具人 
曾几何时，对于Java的序列化的认知一直停留在：「实现个 ```text 
  Serializbale
  ``` 接口」不就好了的状态，直到 ... 
...
试想，如果上面在定义 ```text 
  ...
  ``` 时会发生什么呢？ 
实验结果是：此时的程序运行会报错，并抛出 ```text 
  NotSerializableException
  ``` 异常： 
![Test](https://codesheep.oss-cn-hangzhou.aliyuncs.com/blog/20200513163224.png  '序列化-反序列化，我忍你很久了') 
我们按照错误提示，由源码一直跟到 ```text 
  ObjectOutputStream
  ``` 的 ```java 
  writeObject0()
  ``` 方法底层一看，才恍然大悟： 
![Test](https://codesheep.oss-cn-hangzhou.aliyuncs.com/blog/20200513163224.png  '序列化-反序列化，我忍你很久了') 
如果一个对象既不是字符串、数组、枚举，而且也没有实现 ```text 
  Serializable
  ``` 接口的话，在序列化时就会抛出 ```text 
  NotSerializableException
  ``` 异常！ 
...
###  ```text 
  serialVersionUID
  ``` 号有何用？ 
相信你一定经常看到有些类中定义了如下代码行，即定义了一个名为 ```text 
  serialVersionUID
  ``` 的字段： 
 ```java 
  ...
  ```  
你知道这句声明的含义吗？为什么要搞一个名为 ```text 
  serialVersionUID
  ``` 的序列号？ 
继续来做一个简单实验，还拿上面的 ```text 
  Student
  ``` 类为例，我们并没有人为在里面显式地声明一个 ```text 
  serialVersionUID
  ``` 字段。 
我们首先还是调用上面的 ```java 
  serialize()
...
接下来我们在 ```text 
  Student
  ``` 类里面动点手脚，比如在里面再增加一个名为 ```text 
  studentID
  ``` 的字段，表示学生学号： 
![Test](https://codesheep.oss-cn-hangzhou.aliyuncs.com/blog/20200513163224.png  '序列化-反序列化，我忍你很久了') 
这时候，我们拿刚才已经序列化到本地的 ```text 
  student.txt
  ``` 文件，还用如下代码进行反序列化，试图还原出刚才那个 ```text 
  Student
  ``` 对象： 
 ```java 
  public static void deserialize(  ) throws IOException, ClassNotFoundException {
...
运行发现报错了，并且抛出了 ```text 
  ...
  ``` 异常： 
![Test](https://codesheep.oss-cn-hangzhou.aliyuncs.com/blog/20200513163224.png  '序列化-反序列化，我忍你很久了') 
这地方提示的信息非常明确了：序列化前后的 ```text 
  serialVersionUID
  ``` 号码不兼容！ 
从这地方最起码可以得出两个重要信息： 
 
  1、serialVersionUID是序列化前后的唯一标识符  
  2、默认如果没有人为显式定义过 ```text 
  serialVersionUID
  ``` ，那编译器会为它自动声明一个！  
...
第2个问题： 如果在定义一个可序列化的类时，没有人为显式地给它定义一个 ```text 
  serialVersionUID
  ``` 的话，则Java运行时环境会根据该类的各方面信息自动地为它生成一个默认的 ```text 
  serialVersionUID
  ``` ，一旦像上面一样更改了类的结构或者信息，则类的 ```text 
  serialVersionUID
  ``` 也会跟着变化！ 
所以，为了 ```text 
  serialVersionUID
  ...
  implements Serializable
  ``` 的类，都最好人为显式地为它声明一个 ```text 
  serialVersionUID
  ``` 明确值！ 
当然，如果不想手动赋值，你也可以借助IDE的自动添加功能，比如我使用的 ```text 
  IntelliJ IDEA
  ``` ，按 ```text 
  alt + enter
  ``` 就可以为类自动生成和添加 ```text 
...
对于第一点，因为序列化保存的是对象的状态而非类的状态，所以会忽略 ```text 
  static
  ``` 静态域也是理所应当的。 
对于第二点，就需要了解一下 ```text 
  transient
  ``` 修饰符的作用了。 
如果在序列化某个类的对象时，就是不希望某个字段被序列化（比如这个字段存放的是隐私值，如： ```text 
  密码
  ``` 等），那这时就可以用 ```text 
  transient
  ``` 修饰符来修饰该字段。 
...
比如我故意将学生的分数改为 ```text 
  ...
  ``` ，此时反序列化立马终止并且报错： 
![Test](https://codesheep.oss-cn-hangzhou.aliyuncs.com/blog/20200513163224.png  '序列化-反序列化，我忍你很久了') 
对于上面的代码，有些小伙伴可能会好奇，为什么自定义的 ```text 
  private
  ``` 的 ```java 
  readObject()
  ``` 方法可以被自动调用，这就需要你跟一下底层源码来一探究竟了，我帮你跟到了 ```text 
  ObjectStreamClass
  ``` 类的最底层，看到这里我相信你一定恍然大悟： 
![Test](https://codesheep.oss-cn-hangzhou.aliyuncs.com/blog/20200513163224.png  '序列化-反序列化，我忍你很久了') 
又是反射机制在起作用！是的，在Java里，果然万物皆可“反射”（滑稽），即使是类中定义的 ```text 
  private
