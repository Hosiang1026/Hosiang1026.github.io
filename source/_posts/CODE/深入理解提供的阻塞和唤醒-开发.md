...
点击上方蓝字 ↑↑ Throwable文摘 <!-- more -->
  ...
  ``` 的源码实现，再结合很久之前看过的一篇关于 ```text 
  Object
  ``` 提供的等待和唤醒机制的 ```text 
  JVM
  ``` 实现，发现两者有不少的关联，于是决定重新研读一下 ```text 
  Object
  ``` 中提供的阻塞和唤醒方法。本文阅读 ```text 
  JDK
  ``` 类库源码使用的 ```text 
  JDK
  ``` 版本是 ```text 
  ...
  #### Object提供的阻塞和唤醒API 
   ```text 
  java.lang.Object
  ``` 作为所有非基本类型的基类，也就是说所有 ```text 
  java.lang.Object
  ``` 的子类都具备阻塞和唤醒的功能。下面详细分析 ```text 
  Object
  ``` 提供的阻塞和唤醒 ```text 
  API
  ``` ，它们有一共共同特点：必须在 ```text 
  ...
  对于同步代码块而言， ```text 
  synchronized
  ``` 关键字抽象到字节码层面就是同步代码块中的字节码执行在 ```text 
  monitorenter
  ``` 和 ```text 
  monitorexit
  ``` 指令之间： 
  ...
  ``` ）与之相关联，当且仅当一个 ```text 
  monitor
  ``` 被持有之后，它将处于锁定状态。线程执行到 ```text 
  monitorenter
  ``` 指令时，将会尝试获取对象所对应的 ```text 
  monitor
  ``` 所有权，即尝试获取对象的锁。 
  对于同步（静态）方法而言， ```text 
  synchronized
  ``` 方法则会被翻译成普通的方法调用和返回指令，如： ```text 
  invokevirtual
  ``` 等等，在 ```text 
  JVM
  ``` 字节码层面并没有任何特别的指令来实现被 ```text 
  synchronized
  ``` 修饰的方法，而是在 ```text 
  Class
  ``` 文件的方法表中将该方法的 ```text 
  ...
  ``` 字段中的 ```text 
  synchronized
  ``` 标志位置1，表示该方法是同步方法并使用调用该方法的对象或该方法所属的 ```text 
  Class
  ``` 在 ```text 
  JVM
  ``` 的内部对象表示 ```text 
  ...
  鉴于笔者 ```text 
  C
  ``` 语言学得不好，这里就无法深入分析 ```text 
  JVM
  ``` 源码的实现，只能结合一些现有的资料和自己的理解重新梳理一下 ```text 
  Object
  ``` 提供的阻塞和唤醒机制这些知识点。结合之前看过 ```text 
  JUC
  ``` 同步器的源码，一时醒悟过来， ```text 
  JUC
  ``` 同步器只是在数据结构和算法层面使用 ```text 
  Java
  ``` 语言对原来 ```text 
  JVM
  ``` 中 ```text 
  ...
  ``` 语言可以通过 ```text 
  JVM
  ``` 源码研究其实现，对于大部分开发者来说这部分的知识其实是暗箱），除非是特别熟练或者是 ```text 
  JDK
  ``` 版本太低尚未引入 ```text 
  JUC
  ``` 包（ ```text 
  ...
  ``` 方法，其功能有可能受到 ```text 
  JVM
  ``` 版本的影响（有可能带来性能提升这样的正面影响，也有可能是负面影响），另一方面 ```text 
  Object
  ``` 提供的 ```text 
  API
  ``` 其实并不灵活。综合来看，实际开发中更建议使用专门为并发设计的 ```text 
  JUC
  ``` 包中的锁相关类库，例如可重入锁 ```text 
  ReentrantLock
  ``` 。 
