...
我们知道配置  ```text 
  ...
  ```  对象的， ```text 
  SqlSession
  ```  对象的重要程度不言而喻。源码中提到， ```text 
  SqlSession
  ```  是  ```text 
  Mybatis
  ```  运行最重要的一个接口，通过此接口，我们可以进行我们的操作指令，获取  ```text 
  mapper
  ``` ，管理事务等操作。 
官网 给出了一个简单的配置demo，通过  ```text 
...
其实上面扯了那么多，只是想引入一下  ```text 
  XMLMapperBuilder
  ``` 。我们知道，我们的配置（比如Spring中的 application.yml），最后会被解析成  ```text 
  Configuration
  ``` ，而  ```text 
  mapper.xml
  ```  文件正是依据我们的配置来进行读取的，读取到的 xml 将被读取成  ```text 
  Resource
  ```  文件，最后在  ```text 
  SqlSessionFactoryBean
  ```  初始化完毕后、也就是在创建  ```text 
...
![Test](https://oscimg.oschina.net/oscnet/up-7e611b6e9f73e6e7b6fd07077ea9175be14.JPEG  'mybatis源码，从配置到 mappedStatement ——  mapper.xml 是如何被解析的-') 
 ...
 ```text 
  ...
  ```  的  ```xml
<mapper>
  ```  标签进行初步的解析，源码如下：可以看到许多熟悉的身影，比如  ```text 
  namespace
  ``` 、 ```text 
  resultMap
  ``` 、 ```text 
...
在  ```text 
  ...
  ``` ，  ```text 
  DATE
  ```  等等，而我们的 java 类型(  ```text 
  mybatis
  ```  称之为  ```text 
  javaType
  ``` ，或者  ```text 
  javaTypeClass
  ``` )也很多，还包括我们很多的 自定义的  ```text 
  TypeHandler
  ``` ，这里就不赘述了。 
那么必然存在一个问题，如何将它们一一对应上？毫无疑问， ```text 
  JdbcType
  ```  可以被解析为多个  ```text 
  javaTypeClass
  ```  ，如  ```text 
...
刚才提到的  ```text 
  ...
  ``` ，或者  ```text 
  id
  ```  实际上是“一个东西”，可以理解为地址和引用之间的关系。比如说，这个  ```text 
  resultMap
  ```  的  ```text 
  id
  ```  叫做  ```text 
...
其实到现在已经很明了了，所有的  ```text 
  ResultMap
  ```  都会被生成一个独立的数据结构，所以无论怎么嵌套，起码在解析层面，是不会出问题的，它只会保存自己的所有子标签，用  ```text
  List<ResultMapping>
  ```  表示，如果子标签中存在内嵌的  ```text 
  ResultMap
  ``` ，则仅仅保存其  ```text 
...
我们已经对  ```text 
  resultMap
  ```  的解析建立起了清晰的认知，那么此时还有另外一个问题， ```text 
  mybatis
  ```  在对查查询结果集进行解析的时候，是如何使用  ```text 
  resultMap
  ```  的？ 
虽然此部分与  ```text 
  mapper.xml
  ```  无关，但如果无法建立起体系，单纯的  ```text 
  resultMap
  ```  分析只会让人一头雾水。 
实际上在  ```text 
  resultMap
  ```  解析完成后，mybatis 会将其保存在  ```text 
  configuration
  ```  中。 ```text 
  configuration
  ```  前面也提到过，里面保存了 mybatis 的配置，但它不仅如此，它还承担了我们  ```text 
  mybtais
  ```  上下文对象的作用。类似于  ```text 
  spring
  ```  框架中的  ```text 
...
目光来到我们的  ```text 
  ...
  ResultSet
  ```  进行解析。 
解析的核心代码如下（有所删减，后续会有文章专门分析  ```text 
  ResultSetHandler
  ``` ，所以这里只是简单提一下） 
 ```java 
...
Extra： 既然说到了这个  ```text 
  ancestorObjects
  ```  map，顺便提一嘴。我们知道，一对多的映射，比如一条主数据对应10条从数据，在 MySQL 中查询出来时，实际上是有10条的。是我们 mybatis 把这十条合成了一条，那么它是怎么做的呢？实际上也是通过  ```text 
  ancestorObject
  ```  来完成的，不过这里主要讲的是  ```text 
  mapper
  ```  部分相关源码，后续会有文章专门讲  ```text 
  ResultSetHandler
  ```  
...
这两个标签相信大家都熟悉，就是提高sql复用率（个人不喜欢这种写法）。有了前面  ```text 
  resultMap
  ```  的基础，我们很容易猜得到，这些标签在解析完以后，会生成自己一个唯一的  ```text 
  id
  ``` ，然后存到  ```text 
  configuration
  ```  里面。 
...
我们一步步解析，这段代码有三个大的分支 
 ...
 第一个分支顾名思义，解析  ```text 
  ...
  ```  标签用的， 
 第二个则是解析普通节点用的， 
 第三个 else 则是代表解析  ```text 
  Text
  ```  节点、 ```text 
  CDATASection
  ```  节点。 
 
先看看怎么解析普通节点，普通节点将节点拆成子节点，然后循环递归调用自己，这个没什么好说的，前面已经说了各种递归，这个也是一样的道理， ```text 
  mybatis
  ```  解析套娃，核心就是递归。 
 ```text 
  text
  ```  节点的解析也很简单，唯一值得注意的就是  ```text 
  variablesContext
  ```  这个东西，它实际上是从配置文件中来，我们可以通过配置全局  ```text 
  variables
  ```  ，它会在这个时候将其填充进去。 
打个比方，下面这个  ```text 
  testValue
...
解析  ```text
   id="including"
  ``` ，一样的规则，它是个普通节点，循环它所有子节点，第一个子节点是  ```xml
<include refid="select"/>
...
个人不太推荐  ```text 
  selectKey
  ```  的使用，个人感觉类似存储过程...，在一个sql里面做各种事情，例如将某个值赋值为另一个sql的结果，比如插入自增id，或者在插入完毕后， ```sql
SELECT LAST_INSERT_ID() AS xxxx
  ```  将插入主键拿到，这种需求更加推荐通过多个mapper + 业务控制、通过插件、或者修改源码的方式去写。 
从代码也很容易看出来，实际上它就是构建了一个新的查询类型的  ```text 
  mappedStatement
  ``` ，将它存到  ```text 
  configuration
  ```  的  ```text 
...
它保存的map  ```text 
  keyGenerators
  ``` ，id 为当前sql的 id，我们执行这条sql语句之前，只需要根据当前执行sql的id，就可以拿到  ```text 
  selectKey
  ```  语句。 
 ```java 
