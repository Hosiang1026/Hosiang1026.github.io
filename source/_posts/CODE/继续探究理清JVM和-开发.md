...
### 一、OOM的认识 ### StackOverflowError ```java public static void main(String[] args) { stackOverflowError(); //Exception in thread "main" java.lang.
...
制码
  `### OutOfMemeoryError：java heap space```java
    String str = "cbuc";
    for (; ; ) {
...
### 二十一、G1特点
 ...
 宏观上看G1之中不再区分年轻代和老年代。把内存划分成多个独立的子域（Region） 
