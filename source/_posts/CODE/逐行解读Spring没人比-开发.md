...
```java
一、前言 这一篇博文主要讲一下我们spring是怎么解决循环依赖的问题的。 二、什么是循环依赖 首先我们需要明确，什么是循环依赖呢？这里举一个简单的例子： @Service public class A { @Aut...
```
<!-- more -->
...
结合我们上篇博文的bean生命周期的知识，我们来模拟一下这两个bean创建的流程： 
 ...
 ![Test](https://api.opics.org/api  '逐行解读Spring- 没人比我更懂循环依赖') 
   
...
由于之前我们已经详细讲过整个bean的生命周期了，所以这里就只挑三级缓存相关的代码段来讲了，会跳过比较多的代码，同学们如果有点懵，可温一下万字长文讲透bean的生命周期。 
2.1. Spring的三级缓存设计 
2.1.1. 三级缓存源码 
...
复制代码
  `也就是说，我们伪代码的这一块在spring里面也有对应的体现，完美：```java
复制代码
  ``` 
...
复制代码
  `我们看一下这个 isSingletonCurrentlyInCreation的逻辑：```java
```java
...
```
复制代码
  `所以，连我们都无法解决的问题，就不应该强求spring来解决了吧~```java
    }
    }
复制代码
  `启动之后，果然报错了：```text
