...
##### 进程关系链 
进程是家族式管理的,父子关系,兄弟关系,朋友关系,子女关系,甚至陌生人关系(等待你消亡)在一个进程的生命周期中都会记录下来.用什么来记录呢?当然是内核最重要的胶水结构体 ```text 
  LOS_DL_LIST
  ``` ,进程控制块(以下简称 ```text 
  PCB
...
解读 
  ...
  ```text 
  childrenList
  ``` 孩子链表,所有由它fork出来的进程都挂到这个链表上.上面的孩子进程在死亡前会将自己从上面摘出去,转而挂到 ```text 
  exitChildList
  ``` 链表上. 
  ```text 
  exitChildList
...
解读 
 ...
 将自己变成了 ```text 
  ...
  ``` 僵尸进程. 
 老父亲跑到村口广播这个孩子已经死亡的信号 ```text 
  OsKill
  ``` . 
 将自己挂入进程回收链表,等待回收任务 ```text 
  ResourcesTask
  ``` 回收资源. 
 最后删除这个正在运行的任务,很明显其中一定会发生一次调度 ```text 
  OsSchedResched
  ``` .  ```java 
    //删除一个正在运行的任务
  ...
  ```   
 但这是一个自然死亡的进程,还有很多非正常死亡在其他篇幅中已有说明.请自行翻看.非正常死亡的会产生僵尸进程.这种进程需要别的进程通过  ```text 
  waitpid
  ``` 来回收. 
...
##### 僵尸进程 
一个进程在终止时会关闭所有文件描述符，释放在用户空间分配的内存，但它的 ```text 
  PCB
  ``` 还保留着，内核在其中保存了一些信息：如果是正常终止则保存着退出状态，如果是异常终止则保存着导致该进程终止的信号是哪个。这个进程的父进程可以调用wait或waitpid获取这些信息，然后彻底清除掉这个进程。 
如果一个进程已经终止，但是它的父进程尚未调用wait或waitpid对它进行清理，这时的进程状态称为僵尸（Zombie）进程,即 Z 进程.任何进程在刚终止时都是僵尸进程，正常情况下，僵尸进程都立刻被父进程清理了. 不正常情况下就需要手动 ```text 
  waitpid
  ``` 清理了. 
##### waitpid 
在鸿蒙系统中，一个进程结束了，但是它的父进程没有等待（调用 ```text 
  wait
  ```   ```text 
  waitpid
  ``` ）它，那么它将变成一个僵尸进程。通过系统调用  ```text 
  waitpid
  ``` 可以彻底的清理掉子进程.归还 ```text 
  pcb
  ``` .最终调用到 ```text 
...
解读 
  ...
  */
  ...
  ``` 是行为参数,提供了一些另外的选项来控制waitpid()函数的行为。 
   ...
   鸿蒙目前只支持了LOS_WAIT_WNOHANG模式,内核源码中虽有 ```text 
  LOS_WAIT_WUNTRACED
  ``` 和 ```text 
  LOS_WAIT_WCONTINUED
...
##### 参与贡献 
  ...
  ![Test](https://api.opics.org/api 鸿蒙内核源码分析(进程回收篇) - 进程在临终前如何向老祖宗托孤 - 百篇博客分析HarmonyOS源码 - v47.02)  
