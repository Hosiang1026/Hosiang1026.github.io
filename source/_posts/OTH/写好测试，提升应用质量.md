...
&emsp;&emsp;相信在国内一些中小型公司，开发者很少会去写软件测试相关的代码。当然这背后有一些原因在。本文就讲讲 iOS 开发中的软件测试相关的内容。 一、 测试的重要性 测试很重要！测试很重要！测试很...
<!-- more -->
...
- (void)testShouldIEatSomethingWhenFull
{
  ...
  ```  
##### 3. 明确标识被测试类 
这条主要站在团队合作和代码可读性角度出发来说明。写过单元测试的人都知道，可能某个函数本来就10行代码，可是为了测试它，测试代码写了30行。一个方法这样写问题不大，多看看就看明白是在测试哪个类的哪个方法。可是当这个类本身就很大，测试代码很大的情况下，不管是作者自身还是多年后负责维护的其他同事，看这个代码阅读成本会很大，需要先看测试文件名  ```java 
  代码类名 + Test
  ```  才知道是测试的是哪个类，看测试方法名  ```java 
  test + 方法名
  ```  才知道是测试的是哪个方法。 
这样的代码可读性很差，所以应该为当前的测试对象特殊标记，这样测试代码可读性越强、阅读成本越低。比如定义局部变量  ```java 
  _sut
...
@end
  ...
  ```  
##### 4. 使用分类来暴露私有方法、私有变量 
某些场景下写的测试方法内部可能需要调用被测对象的私有方法，也可能需要访问被测对象的某个私有属性。但是测试类里面是访问不到被测类的私有属性和私有方法的，借助于  ```java 
  Category
  ```  可以实现这样的需求。 
为测试类添加一个分类，后缀名为  ```java 
  UnitTest
  ``` 。如下所示 
 ```java 
  HermesClient
...
##### 1. TDD 
TDD 强调不断的测试推动代码的开发，这样 ```java 
  简化了
  ``` 代码，保证了代码质量。 
思想是在拿到一个新的功能时，首先思考该功能如何测试，各种测试用例、各种边界 case；然后完成测试代码的开发；最后编写相应的代码以满足、通过这些测试用例。 
TDD 开发过程类似下图： 
<a name="TDDStructure"></a>![Test](https://raw.githubusercontent.com/FantasticLBP/knowledge-kit/master/assets/2020-07-13-TDDStructure.png  '写好测试，提升应用质量') 
...
如何开展 TDD** 
  ...
  新建一个工程，确保 “Include Unit Tests” 选项是选中的状态 ![Test](https://raw.githubusercontent.com/FantasticLBP/knowledge-kit/master/assets/2020-07-13-TDDStructure.png  '写好测试，提升应用质量')  
  创建后的工程目录如下 ![Test](https://raw.githubusercontent.com/FantasticLBP/knowledge-kit/master/assets/2020-07-13-TDDStructure.png  '写好测试，提升应用质量')  
  删除 Xcode 创建的测试模版文件  ```java 
  TDDDemoTests.m
  ```   
  ...
  实现测试用例代码。创建继承自 Unit Test Case class 的测试类，命名为  ```java 
  工程前缀+测试类名+Test
  ``` ，也就是  ```java 
  TDDPersonTest.m
  ``` 。 ![Test](https://raw.githubusercontent.com/FantasticLBP/knowledge-kit/master/assets/2020-07-13-TDDStructure.png  '写好测试，提升应用质量')  
  因为要测试 Person 类，所以在主工程中创建 Person 类  
  因为要测试人类在吃饭后说一句“好饱啊”。所以设想那个类目前只有一个吃饭的方法。于是在  ```java 
  TDDPersonTest.m
  ```  中创建一个测试函数  ```java 
...
Xcode 自带的测试系统是  ```java 
  ...
  为测试类每个方法写的测试方法有一定的规范。命名必须是  ```java 
  test+被测方法名
  ``` 。函数无参数、无返回值。比如  ```java 
...
##### 3. 测试框架 
###### 1. Kiwi 
BDD 框架里的 Kiwi 可圈可点。使用 CocoaPods 引入  ```java 
  pod 'Kiwi'
  ``` 。看下面的例子 
...
针对异步测试，XCTest 则需要创建一个  ```java 
  XCTestExpectation
  ```  对象，在异步实现里面调用该对象的  ```java 
  fulfill
...
iOS 中很多网络都是基于 NSURL 系统下的类实现的。所以我们可以利用  ```java 
  NSURLProtocol
...
紧接着开始发送网络请求。实际上在  ```java 
  - (void)startLoading
  ```  方法中可以用任何网络能力去完成请求，比如 NSURLSession、NSURLConnection、AFNetworking 或其他网络框架。OHHTTPStubs 的做法是获取 request、client 对象。如果 HTTPStubs 单例中包含  ```java 
...
#### 六、UI 测试 
上面文章大篇幅的讲了单元测试相关的话题，单元测试十分适合代码质量、逻辑、网络等内容的测试，但是针对最终产物 App 来说单元测试就不太适合了，如果测试 UI 界面的正确性、功能是否正确显然就不太适合了。Apple 在 Xcode 7 开始推出的  ```java 
  UI Testing
  ```  就是苹果自己的 UI 测试框架。 
很多 UI 自动化测试框架的底层实现都依赖于  ```java 
  Accessibility
  ``` ，也就是 App 可用性。 ```java 
  UI Accessibility
  ```  是 iOS 3.0 引入的一个人性化功能，帮助身体不便的人士方便使用 App。 
Accessibility 通过对 UI 元素进行分类和标记。分类成类似按钮、文本框、文本等类型，使用 identifier 来区分不同 UI 元素。无痕埋点的设计与实现里面也使用  ```java 
...
- (XCUIElementQuery *)childrenMatchingType:(XCUIElementType)type;
  ...
  ```  类似， ```java 
  XCUIElement
  ```  不能直接访问 UI 元素，它是 UI 元素在测试框架中的代理。可以通过  ```java 
  Accessibility
  ```  中的  ```java 
  frame
  ``` 、 ```java 
...
对比很多自动化测试框架都需要找出 UI 元素，也就是借助于  ```java 
  Accessibility
  ```  的  ```java 
  identifier
...
#### 七、 测试经验总结 
TDD 写好测试再写业务代码，BDD 先写实现代码，再写基于行为的测试代码。另一种思路是没必要针对每个类的私有方法或者每个方法进行测试，因为等全部功能做完后针对每个类的接口测试，一般会覆盖据大多数的方法。等测试完看如果方法未被覆盖，则针对性的补充  ```java 
  Unit Test
  ``` 。 
