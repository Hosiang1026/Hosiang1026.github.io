...
apply plugin: 'com.huawei.agconnect'  // HUAWEI agconnect Gradle plugin
  `3. 在app的build中配置签名文件并将签名文件(xxx.jks)放入app目录下：```java
  signingConfigs {
    release {
...
```
  `3. 覆写“onActivityResult”方法，用于处理语音识别服务返回结果。```java
```
  @Override
...
public static void verifyStoragePermissions(Activity activity) {
  ...
  `2. 新建音频文件转写引擎并初始化；新建音频文件转写配置器。```java
  // 设置 ApiKey.
