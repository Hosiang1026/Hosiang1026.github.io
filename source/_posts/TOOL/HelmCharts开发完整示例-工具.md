...
lm 的使用是比较简单的，但是要让我们自己开发一个 Chart 包还是有不小难度的，主要还是 go template 的语法规则不够人性化，这里我们用一个完整的实例来演示下如何开发一个 Helm Chart 包。..
<!-- more -->
...
<nodeip>:31950
 ...
 ghost 
 看上去要部署 Ghost 是非常简单的，但是如果我们需要针对不同的环境进行不同的设置呢？比如我们想将它部署到不同环境（staging、prod）中去，是不是我们需要一遍又一遍地复制我们的 Kubernetes 资源清单文件，这只是一个场景，还有很多场景可能需要我们去部署应用，这种方式维护起来是非常困难的，这个时候就可以理由 Helm 来解放我们了。 
...
3 directories, 10 files
  ...
  `该命令会创建一个默认 Helm Chart 包的脚手架，可以删掉下面的这些使用不到的文件：```text
...
            {{- end }}
  ...
  `这和我们前面的资源清单文件非常类似，只是将```text
  replicas
  `的值使用```java
  {{ .Values.replicaCount }}
  `模板来进行替换了，表示会用```text
  replicaCount
  `这个 Values 值进行渲染，然后还可以通过设置环境变量来配置 Ghost，同样修改```text
...
```json
  `需要```text
  service.type
  `为 NodePort 或者 LoadBalancer 并且```text
  service.nodePort
  ``` 
...
app.kubernetes.io/instance: {{ .Release.Name }}
  ...
  `然后我们可以将 Deployment 的名称和标签替换掉：```java
...
    # other spec...
  ...
  `为 Deployment 增加 label 标签，同样 labelSelector 中也使用```text
  my-ghost.selectorLabels
  `这个命名模板进行替换，同样对 Service 也做相应的改造：```java
spec:
...
```
  ...
  `现在我们再为 Ghost 添加一个 Ingress 的模板，新建```text
  `模板文件，先添加一个 v1 版本的 Ingress 模板：```text
spec:
...
```
  ...
  `然后接下来我们来兼容下其他的版本格式，这里需要用到```text
  Capabilities
  `对象，在 Chart 包的```text
  _helpers.tpl
  `文件中添加几个用于判断集群版本或 API 的命名模板：```json
```python
...
```
  ...
  `来判断我们应该使用的 APIVersion，如果版本为```text
  networking.k8s.io/v1
  `，则定义为```text
  isStable
  `，此外还根据版本来判断是否需要支持```text
  pathType
  ``` 
...
          servicePort: {{ .Values.service.port }}
  ...
  `由于有的场景下面并不需要使用 Ingress 来暴露服务，所以首先我们通过一个```text
  ingress.enabled
  `属性来控制是否需要渲染，然后定义了一个```text
  $apiIsStable
  ``` 
  变量，来表示当前集群是否是稳定版本的 API，然后需要根据该变量去渲染不同的属性，比如对于 ingressClass，如果是稳定版本的 API 则是通过  
  ...
  `这个 annotations 来指定。然后这里我们在```text
  values.yaml
  `文件中添加如下所示默认的 Ingress 的配置数据：```text
  ingress:
```
  enabled: true
  ingressClass: nginx
```
  ...
  `现在我们再次渲染 Helm Chart 模板来验证资源清单数据：```text
---
spec:
---
spec:
...
            port:
  ...
  `从上面的资源清单可以看出是符合我们的预期要求的，我们可以来安装测试下结果：```xml
...
my-ghost   nginx   ghost.k8s.local   192.168.31.31   80      3m32s
  ...
  `正常就可以部署成功 Ghost 了，并且可以通过域名```text
ghost.k8s.local
  ``` 
  进行访问了： 
...
tolerations: {{- toYaml . | nindent 8 }}
  ...
  `接下来当然就是镜像的配置了，如果是私有仓库还需要指定```text
  imagePullSecrets
  `：```json
...
  #   - myRegistryKeySecretName
  ...
  `然后就是 resource 资源声明，这里我们定义一个默认的 resources 值，同样用```text
  `函数来控制空格：```java
{{ toYaml .Values.resources | indent 10 }}
 ...
 最后是健康检查部分，虽然我们之前没有做 livenessProbe，但是我们开发 Chart 模板的时候就要尽可能考虑周全一点，这里我们加上存活性和可读性、启动三个探针，并且根据  
  ...
  `三个 Values 值来判断是否需要添加探针，探针对应的参数也都通过 Values 值来配置：```java
```
...
generated: "2022-03-17T17:40:21.090371+08:00"
  ...
  `该```text
  `文件是我们通过仓库获取 Chart 包的关键。然后将代码推送到 GitHub：```xml
  ➜ git status
On branch main
...
   9c389a6..aae1059  main -> main
  ...
  `接下来为该仓库设置 GitHub Pages，首先在本地新建一个```text
  gh-pages
  `分支：```text
  ...
  `文件保留到根目录下面，其他文件忽略，然后推送到远程仓库：```text
