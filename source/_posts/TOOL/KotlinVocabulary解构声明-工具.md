...
<!-- more -->
                                                                                                                                                                                        ...
                                                                                                                                                                                        ![Test](https://api.opics.org/api  'Kotlin Vocabulary - 解构声明详解') 
...
解构不允许您选择使用某个确切的字段；它永远使用前 x 个字段，这里的 x 是您所声明的变量数。这样做的缺点是很容易造成错误，比如下面这段代码便可能造成意外的结果: `val (name, rating) = goodDoggo` rating 值事实上会持有 goodDoggo.breed 的值。您将会看到一个警告: "Variable name ‘rating’ matches the name of a different component" (‘rating’ 变量名匹配了名字不同的 component) 并且建议您将 rating 重命名为 breed。于这个警告只存在于 IDE 中，而且不是编译器警告，您很容易就会注意不到它: 
 
如果您只需要一部分不连续的字段，可以使用 _ 代替那些您不感兴趣的字段，Kotlin 将会跳过它们。这样一来示例就会变成下面这样: `val (name, _, rating) = goodDoggo` #### 内部原理
