...
### 二、Math operands should be cast before assignment-数字操作在操作或赋值前要分配
...
### 三、Strings and Boxed types should be compared using "equals()"

#### 字符串和包装类型对比时应该使用equals方法。

使用引用相等==或！=比较java.lang.String或包装类型（如java.lang.Integer）的两个实例几乎总是false，因为它不是比较实际值，而是在内存中的位置。
不合格的代码
...
### 四、Boxing and unboxing should not be immediately reversed
...
### 五、Intermediate Stream methods should not be left unused
...
### 六、Loops with at most one iteration should be refactored
...
### 七、Non-thread-safe fields should not be static
...
### 八、"InterruptedException" should not be ignored
