...
#### TCP
TCP 全称为传输控制协议（Transmission Control Protocol），它由 IETF 的 RFC 793 义是一种面向连接的点对点传输通信协议，它以有序顺序将数据包作为非结构化字节流发送。 
...
假设有两座房子House1 和 House2，并且必须从 H1 发送一封信到 H2。但是这两座房子之间有一条河。现在我们如何寄信？ 
解决方案 1：在河上架桥，然后将其交付。 
解决方案 2：通过鸽子运送。 
