...
`Option` 类型的变量只能被赋值为 `'A'`、`'B'`、`'C'`、`'D'` 中一个值。这一种用法和枚举很像，比如：
...
有一些类型是无法交叉的，比如基本类型。示例：`type A = string & number` 因为一个变量不可能既是字符串又是数字，所以最终 `A` 类型是 `never` 类型。
...
let merge = new Merge<string, number>('a', 0)
```
...
虽然我们可能心里知道在使用时我们会传入一个具有 `length` 成的变量（比如字符串），但是在编译时编译器并不知道。因此，我们需要告诉编译器 `T` 类型是那一类具有 `length` 成员的类型。使用 `extends` 关键字：
