...
除了必需的信息外，还建议包含项目的正确依赖关系，以便构建工具可以使用该信息正确地解决传递依赖关系，并且不需要用户手动管理依赖关系。 
 ...
 项目坐标信息，也叫做GAV。包括groupId ，artifactId和version，如下所示： 
  ...
  `除此之外，如果项目不是jar包，还需要包含packaging信息，有效的值包括：```text
  jar
  `,```text
  war
  ear
...
署成后，状态会变成Open，点击close会触发对组件的校验，如果校验成功，那么可以点击release按钮将其部署到中央仓库中。 
