...
那么Web服务器是如何记住一些事情呢？既然Web服务器记不住东西，那么我们就在外部想办法记住，相当于服务器给每个客户端都贴上了一个小纸条。上面记录了服务器给我们返回的一些信息。然后服务器看到这张小纸条就知道我们是谁了。那么 ```text 
  Cookie
  ``` 是谁产生的呢？Cookies是由服务器产生的。接下来我们描述一下 ```text 
  Cookie
  ``` 产生的过程 
 
 浏览器第一次访问服务端时，服务器此时肯定不知道他的身份，所以创建一个特的身份标识数据，格式为 ```java 
  key=value
  ``` ，放入到 ```text 
  Set-Cookie
  ``` 字段里，随着响应报文发给浏览器。 
...
localhost:8005/testCookies
  ...
  ``` 值正是我们服务器中设置的值。 
![Test](https://api.opics.org/api  '一文彻底搞懂Cookie-Session-Token到底是什么') 
接下来我们再次刷新这个页面可以看到在请求体中已经设置了 ```text 
  Cookie
  ``` 字段，并且将我们的值也带过去了。这样服务器就能够根据 ```text 
  Cookie
  ``` 中的值记住我们的信息了。 
![Test](https://api.opics.org/api  '一文彻底搞懂Cookie-Session-Token到底是什么') 
接下来我们换一个请求呢？是不是 ```text 
  Cookie
  ``` 也会带过去呢？接下来我们输入路径 ```text
localhost:8005
  ``` 请求。我们可以看到 ```text 
  Cookie
  ``` 字段还是被带过去了。 
...
说到这里，应该知道了 ```text 
  Cookie
  ``` 就是服务器委托浏览器存储在客户端里的一些数据，而这些数据通常都会记录用户的关键识别信息。所以 ```text 
  Cookie
  ``` 需要用一些其他的手段用来保护，防止外泄或者窃取，这些手段就是 ```text 
  Cookie
  ``` 的属性。 
...
设置为 ```java 
  ...
  ``` ，接下来我们访问 ```text
localhost:8005/testCookies
  ``` ，我们可以看到在左边和我们指定的路径是一样的，所以 ```text 
  Cookie
  ``` 才在请求头中出现，接下来我们访问 ```text
localhost:8005
  ``` ，我们发现没有 ```text 
  Cookie
  ``` 字段了，这就是 ```text 
...
172.16.42.81:8005/testCookies
  ...
  ``` 。 
![Test](https://api.opics.org/api  '一文彻底搞懂Cookie-Session-Token到底是什么') 
接下来的几个参数就不一一演示了，相信到这里大家应该对 ```text 
  Cookie
  ``` 有一些了解了。 
#### Session 
> Cookie是存储在客户端方，Session是存储在服务端方，客户端只存储 ```text 
  SessionId
  ```  
在上面我们了解了什么是 ```text 
  Cookie
  ``` ，既然浏览器已经通过 ```text 
  Cookie
  ``` 实现了有状态这一需求，那么为什么又来了一个 ```text 
  Session
  ``` 呢？这里我们想象一下，如果将账户的一些信息都存入 ```text 
  Cookie
  ``` 中的话，一旦信息被拦截，那么我们所有的账户信息都会丢失掉。所以就出现了 ```text 
  Session
  ``` ，在一次会话中将重要信息保存在 ```text 
  Session
  ``` 中，浏览器只记录 ```text 
...
localhost:8005/testGetSession
 ...
 客户端：和 ```text 
  Cookie
  ``` 过期一致，如果没设置，默认是关了浏览器就没了，即再打开浏览器的时候初次请求头中是没有 ```text 
  SessionId
  ``` 了。 
 服务端：服务端的过期是真的过期，即服务器端的 ```text 
  Session
...
既然我们知道了 ```text 
  Session
  ``` 是在服务端进行管理的，那么或许你们看到这有几个疑问， ```text 
  Session
  ``` 是在在哪创建的？ ```text 
  Session
  ``` 是存储在什么数据结构中？接下来带领大家一起看一下 ```text 
  Session
  ``` 是如何被管理的。 
 ```text 
  Session
  ``` 的管理是在容器中被管理的，什么是容器呢？ ```text 
  Tomcat
  ``` 、 ```text 
  Jetty
  ``` 等都是容器。接下来我们拿最常用的 ```text 
  Tomcat
  ``` 为例来下 ```text 
  Tomcat
  ``` 是如何管理 ```text 
...
#### Token 
 ```text 
  Session
  ``` 是将要验证的信息存储在服务端，并以 ```text 
  SessionId
  ``` 和数据进行对应， ```text 
  SessionId
  ``` 由客户端存储，在请求时将 ```text 
  SessionId
  ``` 也带过去，因此实现了状态的对应。而 ```text 
  Token
...
>  ```text 
  ...
  ``` 相比较于 ```text 
  Session
  ``` 的优点在于，当后端系统有多台时，由于是客户端访问时直接带着数据因此无需做共享数据的操作。 
##### Token的优点 
...
##### JWT的结构 
实际的JWT大概长下面的这样，它是一个很长的字符串，中间用 ```text 
  .
  ``` 分割成三部分 
![Test](https://api.opics.org/api  '一文彻底搞懂Cookie-Session-Token到底是什么') 
JWT是有三部分组成的 
 ...
 ```json
{
  ...
  ```  
默认情况下JWT是不加密的，任何人只要在网上进行Base64解码就可以读到信息，所以一般不要将秘密信息放在这个部分。这个Json对象也要用 ```text 
  Base64URL 
  ``` 算法转成字符串 
###### Signature 
Signature部分是对前面的两部分的数据进行签名，防止数据篡改。 
首先需要定义一个秘钥，这个秘钥只有服务器才知道，不能泄露给用户，然后使用Header中指定的签名算法(默认情况是HMAC SHA256)，算出签名以后将Header、Payload、Signature三部分拼成一个字符串，每个部分用 ```text 
  .
  ``` 分割开来，就可以返给用户了。 
...
相信大家看到这应该对 ```text 
 ...
 Cookie是存储在客户端的 
 Session是存储在服务端的，可以理解为一个状态列表。拥有一个唯一会话标识 ```text 
  SessionId
  ``` 。可以根据 ```text 
  SessionId
  ``` 在服务端查询到存储的信息。 
