...
【飞桨开发者说】钟山，中科院信工所工程师，主要研究计算机视觉、深度学习。 下载安装命令 ## CPU版本安装命令 pip install -f https://paddlepaddle.org.cn/pip/oschina/cpu paddlepaddle ...
<!-- more -->
...
```
  ...
  `上面我们定义好了VGG网络结构，这里我们使用定义好的网络来获取分类器。```java
  # 获取分类器
```
...
accuracy =fluid.layers.accuracy(input=predict, label=label) 
  ...
  `为了区别测试和训练，在这里我们克隆一个test_program()。```java
...
```python
    ...
    test_acc = (sum(test_accs) /len(test_accs))  #计算准确率平均值（ 准确率的和/准确率的数）
...
```
  ...
  `得到各个标签的概率值后，获取概率最大的标签，即为颜值的预测结果。```java
  # 获取概率最大的label
```python
