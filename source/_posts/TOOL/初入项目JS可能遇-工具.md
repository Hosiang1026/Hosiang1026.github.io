...
const status) => {
  ...
  `虽然只有六个分支，但是已经看起来臃肿不堪了，可能我们可以把某一个else if变得厚重一下？```javascript
  } else if(status === 2 || status === 222){
  }
  `看起来可能好一些，不过更多的人应该会选择switch case，那么它就会变成：```javascript
  switch(status){
    case 1:
      break;
    case 2:
