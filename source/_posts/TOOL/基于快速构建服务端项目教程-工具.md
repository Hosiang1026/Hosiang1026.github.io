...
```
  ...
  `控制器还有一个```text
  index
  ```  的方法，这个方法被使用 @Action 标记为了可访问的页面，而这个页面又使用 view 视图渲染了 `index.html` 模板（这里模板的扩展名因安全问题被省略）。 
...
##### 2.1 首先我们开启路由组
在  `config/application.conf`  找到配置项  `route.groups`  修改为： `route.groups = API:path:API` 那我这里解释一下 API:path:API 的含义，{路由组的KEY}:{访问路由组的方式}:{路由组的自定义值}，那通过上面这个设置后我们的 API 访问地址前缀就应该是： 
 `127.0.0.1:8080/API/`  
如果我们设置为 `route.groups = API:domain:API.example.com` 那我们访问 API 地址的前缀就： 
 `API.example.com/`  
##### 2.2 我们的表结构设计
