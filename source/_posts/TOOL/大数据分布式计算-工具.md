...
### 一、概述 Apache Spark 是一个快速且通用的集群计算系统。提供 Java、Scala、Python 和 R 中的高级 API，以及支持通用执行图的优化引擎
...
val textFile1 = spContext.textFile("hdfs://node1:8088/wordCount/temp/test.txt")
  ...
  `通过已存在的 RDD 经过算子转换生成新的 RDD：```java
  val rdd = textFile.flatMap(_.split(" "))

  `通过已存在的集合创建新的 RDD：```java
...
#### 3.3 RDD 持久化
```
Spark 中最重要的功能之一就是将数据集持久化（或缓存）在内存中，当持久化一个 RDD 时，每个节点都会将它计算的任何分区存储在内存中，并在对该数据集（或从它派生的数据集）的其他操作中重用它们。这使得未来的行动更快（通常超过 10 倍）。缓存是迭代算法和快速交互使用的关键工具。 通过使用 `persist()` 或 `cache()` 方法将 RDD 标记为持久化，在第一次在 Action 计算时触发，它将计算的结果保存在节点的内存中。在触发后面的 Action 时，该 RDD 将会被缓存在计算节点的内存中，并供后面重用。  
```
...
每个持久化的 RDD 都可以使用不同的存储级别进行存储，例如：允许将数据集持久化到磁盘上、将其持久化在内存中或者作为序列化的 Java 对象（以节省空间），跨节点复制它。这些通过将  
 `StorageLevel` 
```
  对象传递给 `persist()` 来设置级别。 `cache()` 方法是使用默认存储级别的简写，即  
```
...
#### 3.5 数据混洗
部分 Spark 算子会触发众所周知的混洗（Shuffle）事件。Spark中的混洗机制是用于将数据重新分布，其结果是所有数据将在各个分区间重新分组。一般情况下，混洗需要跨执行器（executor）或跨机器复制数据，这也是混洗操作一般都比较复杂而且开销大的原因。 
##### 基本说明
