...
![Test](https://api.opics.org/api  '实战 - 认识 RecyclerView') 
...
```
 RecyclerView 使用 LayoutManager，它支持纵向滑动的列表和横向滑动的列表，以及交错布局的列和格布局的列表。您还可以创建自定义的 LayoutManager； 
...
<resources>
  ...
  `然后，创建一个类，名字为```text
  Datasource
  `，并且可以接收一个 Context 类型的参数。创建一个叫做```java
  getFlowerList()
  `的函数，它负责返回花的名称列表。```java
...
重写 getItemCount() 
```
RecyclerView 示个列表，所以它需要知道列表里共有多少项。由于 flowerList 就是数据源，所以直接返回它的长度即可。 
