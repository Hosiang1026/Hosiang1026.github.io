...
我之前在
 `Github`
  提交案例代码的时候（
 `github.com/chengxy-nds/Springboot-Notebook`
...
注意：
 ...
 `.Erupt`
```
 文件（一般在项目的工作空间内），获取文件位置方式 `System.getProperty("user.dir")` 再看数据库里创建了16张系统表，其中
```
...
但此时新创建的页面不会显示出来，还需要我们手动做一个映射关系，在菜单维护中自定义个单，类型值一定要为新建的 类名 `Student` 。
