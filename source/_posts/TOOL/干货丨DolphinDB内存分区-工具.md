...
2  5  8  11
3  6  9  12
  `例3. 如果X是元组，那么元组中的每个元素对应表中的一列。```java
  x=(["a","b","c"],[4,5,6])
table(x)
...
SEGMENTED IN-MEMORY TABLE
  `对于内存分区表，不能直接访问，需要使用SQL语句访问。```sql
SELECT * from pt
  ``` 
...
```
 ...
 2.1 加载数据到未分区内存表 
使用loadText函数将文本文件数据导入到未分区的内存表。 `trades=loadText(workDir + "/trades.txt")` 2.2 加载数据到内存分区表 2.2.1 使用ploadText函数将文本文件导入为顺序分区的内存表 
这是将数据导入到内存分区表最简单的方法，但缺乏其它导入方法的某些优点及灵活性。例如，需要导入的文本文件必须小于可用内存；无法使用函数 
 `sortBy!` 
 进行分区内有意义的排序，等等。 `trades = ploadText(workDir + "/trades.txt");` 2.2.2 使用loadTextEx函数将文本文件导入为指定分区方式的表 
这种方法适合下列情况： 
...
![Test](https://api.opics.org/api  'DolphinDB内存分区表的加载和操作') 
...
#### 3. 内存表的数据处理
```
以2.2.3中的分区内存表trades为例，介绍内存表的用法。 `trades = ploadText(workDir + "/trades.txt");` 3.1 插入数据 
```
可以通过以下方法往内存表中插入数据： 
...
2
  `对于未分区表，如果使用tableInsert 函数往表中插入数据，新数据可以用元组的形式表示。```java
```
...
```python
 SQL delete 语句 `DELETE FROM trades where qty3<20;` 2. erase 语句 `trades.erase!(< qty3<30 >);` 3.5 删除列 
通过 drop! 函数删除列： `trades.drop!("qty1");` 3.6 重命名列 
通过 rename! 函数重命名列： `trades.rename!("qty2", "qty2New");` 3.7 查看表结构 
```
通过schema函数查看表的结构： 
...
undef 函数会将命名空间删除，而把变量赋值为NULL仍然保留命名空间。 
 3.9 修改列的数据类型 
通过replaceColumn! 函数可以修改列的数据类型。目前只有未分区的内存表才支持修改列的数据类型，分区内存表不支持该功能。以分区的内存表trades为例，将price1的数据类型修改为FLOAT（目前是DOUBLE）： 
