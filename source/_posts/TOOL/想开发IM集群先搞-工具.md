...
### 一、引言 经常有开发者在纠结怎么开发IM集群，虽然真正的使用人数，可能用个人电脑单机都能支撑。 你也许会说，明明不需要用到IM集群，干吗要自找麻烦
...
如上图所示，一个典型的RPC调用过程是这样（过程序号对应上图中的数字）： 
 ...
 2）客户端存根（client stub）接收到调用后，负责将方法、参数等组成够进行网络传输的消息体（将消息体对象序列化为二进制）； 
