...
defer
  ```  各加入了一项新的机制，使得  ```text
defer
  ```  语句在编译时，编译器会根据不同版本与情况，对每个  ```text
defer
  ```  选择不同的机制，以更轻量的方式运行调用。 
##### 堆上分配 
在 Golang 1.13 之前的版本中，所有  ```text
defer
...
deferprocStack
  ```  实现了在栈上分配的形式来取代  ```text
deferproc
  ``` ，相比后者，栈上分配在函数返回后  ```text
_defer
  ```  便得到释放，省去了内存分配时产生的性能开销，只需适当维护  ```text
_defer
  ```  的链表即可。 
编译器有自己的逻辑去选择使用  ```text
deferproc
  ```  还是  ```text
deferprocStack
  ``` ，大部分情况下都会使用后者，性能会提升约 30%。不过在  ```text
defer
  ```  语句出现在了循环语句里，或者无法执行更高阶的编译器优化时，亦或者同一个函数中使用了过多的  ```text
defer
  ```  时，依然会使用  ```text
deferproc
  ``` 。 
##### 开放编码 
Go 1.14 版本继续加入了开发编码（open coded），该机制会将延迟调用直接插入函数返回之前，省去了运行时的  ```text
deferproc
  ```  或  ```text
deferprocStack
  ```  操作，在运行时的  ```text 
...
该机制还引入了一种元素 —— 延迟比特（defer bit），用于运行时记录每个  ```text
defer
  ```  是否被执行（尤其是在条件判断分支中的  ```text
defer
  ``` ），从而便于判断最后的延迟调用该执行哪些函数。 
延迟比特的原理： 同一个函数内每出现一个  ```text
defer
  ```  都会为其分配 1 个比特，如果被执行到则设为 1，否则设为 0，当到达函数返回之前需要判断延迟调用时，则用掩码判断每个位置的比特，若为 1 则调用延迟函数，否则跳过。 
为了轻量，官方将延迟比特限制为 1 个字节，即 8 个比特，这就是为什么不能超过 8 个  ```text
defer
