...
小Hub领读： 接下来的几天，我们开讲Shiro，从入门到分析、集成、单点登录整合等几篇。今天我们先来认识一下Shiro吧~ 其实Shiro框架并不难，我梳理了一下，你只需要学会以下内容基本就足够了。..
<!-- more -->
...
而构建一个互联网应用，权限校验管理是很重要的安全措施，这其中主要含： 
...
public void hello() {  
  ...
  `JSP Taglig实现，freemarker等类似```xml
