...
在上面介绍 K8s 外部组件的时候已经提到，不同的接口分别提供给不同的组件调用，用于配合实现不同的功能。比如  
 ```text
  ...
  `/```text
  DeleteVolume
  `配合 external-provisioner 实现创建/删除 volume 的功能；```text
  ControllerPublishVolume
  ControllerUnpublishVolume
  ``` 
...
func (dswp *desiredStateOfWorldPopulator) processPodVolumes(
  ...
  `而 reconciler 就是消费者，它主要做了三件事：```java
  unmountVolumes()
  ``` 
```
 ：在 ActualStateOfWorld 中遍历 volume，判断其是否在 DesiredStateOfWorld 中，如果不在，则调用 CSI Node 的接口执行 unmount，并在 ActualStateOfWorld 中记录； `mountAttachVolumes()` ：从 DesiredStateOfWorld 中获取需要被 mount 的 volume，调用 CSI Node 的接口执行 mount 或扩容，并在 ActualStateOfWorld 中做记录； `unmountDetachDevices()` ： 在 ActualStateOfWorld 中遍历 volume，若其已经 attach，但没有使用的 pod，并在 DesiredStateOfWorld 也没有记录，则将其 unmount/detach 掉。 
```
...
```
  ...
  `中完成，再看 operationExecutor 的代码：```java
```go
...
```
  ...
  `这里先去注册到 kubelet 的 CSI 的 plugin 列表中找到对应的插件，然后再执行```text
  volumeMounter.SetUp
  ``` 
...
func (c *csiMountMgr) SetUpAt(dir string, mounterArgs volume.MounterArgs) error {
  ...
  `可以看到，在 kubelet 中调用 CSI Node```text
  NodePublishVolume
  NodeUnPublishVolume
  ``` 
...
```
  ...
  `每当有业务 pod 退出时，CSI Node 会在接口```text
  NodeUnpublishVolume
  ``` 
