...
```
#### 1.2 Mongodb配置优化(网络IO复用，网络IO和磁盘IO做分离)
```
...
但是，从4.2章节优化后的时延可以看出，集群偶尔还是会有抖动，需要进一步优化以消除时延抖动，最终保持时间完全延迟控制在2-4ms，并且无任何超过10ms的抖动。 
