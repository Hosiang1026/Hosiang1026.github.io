...
``` 
 ...
 ```javascript
```javascript
function text: string, author: string, date?: Date) {
return {
text: text,author: author,date: date || new Date()}}
```
...
function text: string, author: string, date: Date = new Date())return {
...
```javascript
function obj: unknown): obj is Product[] {
return Array.isArray(obj) && obj.every(isProduct)}
function isProduct (obj: unknown): obj is Product {
return obj != null&& typeof (obj as Product).id === 'string'}
async function loadProducts(): Promise<Product[]> {
const response = await fetch('https://API.mysite.com/products')const products: unknown = await response.json()if (!isArrayOfProducts(products)) {
