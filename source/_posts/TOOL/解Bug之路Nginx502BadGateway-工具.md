---
title: 解Bug之路Nginx502BadGateway
categories: 其他系列
tags:
  - Shell
abbrlink: da24fb74
...
### 一、解Bug之路-Nginx 502 Bad Gateway
#### 1.1 前言
之前读过一些Linux内核源码，没想到在处理实际问题的时候真的派上用场了。看到报错的那一瞬间，现象、原因、解决方案在脑子里一下子就串起来了，那种感觉特别流畅。这次解决Nginx 502的问题，就是这种感觉。 
#### 1.2 Bug现场
```
这个问题其实不难解决，但引发的现象倒是挺有意思的。最近在对自研的dubbo协议隧道网关做压测（这个网关的设计也挺有意思，后面准备单独写一篇）。压测的拓扑是这样的：![Test](https://oscimg.oschina.net/oscnet/up-cb1706f8f544254d756b32eee5c1aa35c91.png  '解Bug之路-Nginx 502 Bad Gateway ') 为了测试gateway的单机性能，两端各保留一台网关，gateway1和gateway2。压到一定程度就开始报错，压测直接停了。第一反应就是：网关扛不住了。 
```
#### 1.3 网关的情况
```
去Gateway2的机器上看了下，没有任何报错。而Gateway1这边却有大量的502报错。502是Bad Gateway，Nginx的经典报错，第一反应就是Gateway2被压垮了，被Nginx在Upstream中踢掉了。![Test](https://oscimg.oschina.net/oscnet/up-cb1706f8f544254d756b32eee5c1aa35c91.png  '解Bug之路-Nginx 502 Bad Gateway ') 先看看Gateway2的负载情况，查了下监控，发现Gateway2在4核8G的机器上只用了一个核，完全看不出有瓶颈的样子。难道是IO有问题？看了下网卡流量，小得可怜，这个猜想也被排除了。 
```
#### 1.4 Nginx所在机器CPU利用率接近100%
```
...
#### 1.5 去掉Nginx直接通信毫无压力
```
既然猜测是Nginx的瓶颈，就把Nginx去掉吧。Gateway1和Gateway2直连，压测TPS面就飙升了，而且Gateway2的CPU最多也就吃了2个核，毫无压力。 ![Test](https://oscimg.oschina.net/oscnet/up-cb1706f8f544254d756b32eee5c1aa35c91.png  '解Bug之路-Nginx 502 Bad Gateway ') 
```
#### 1.6 去Nginx上看下日志
由于Nginx机器权限不在手上，所以一开始没有关注其日志，现在联系一下对应的运维去看一下。在accesslog里面发现了大量的502报错，确实是Nginx的。又看了下错误日志，发现有大量的 
 `Cannot assign requested address`  
```
之前读过TCP源码，一瞬间就反应过来，是端口号耗尽了！由于Nginx upstream和后端Backend默认是短连接，所以在大量请求流量进来的时候会产生大量TIME_WAIT的连接。 ![Test](https://oscimg.oschina.net/oscnet/up-cb1706f8f544254d756b32eee5c1aa35c91.png  '解Bug之路-Nginx 502 Bad Gateway ') 而这些TIME_WAIT是占据端口号的，而且基本要1分钟左右才能被Kernel回收。 ![Test](https://oscimg.oschina.net/oscnet/up-cb1706f8f544254d756b32eee5c1aa35c91.png  '解Bug之路-Nginx 502 Bad Gateway ') 
```
 ```bash
...
```
#### 1.7 为什么Nginx的CPU是100%
```
也很快想明白了Nginx为什么吃满了机器的CPU，问题就出在端口号的搜索过程。 ![Test](https://oscimg.oschina.net/oscnet/up-cb1706f8f544254d756b32eee5c1aa35c91.png  '解Bug之路-Nginx 502 Bad Gateway ') 看下最耗性能的一段函数： 
```
 ```java
...
```
#### 1.8 为什么Gateway1调用Nginx没有问题
```
很简单，因为在Gateway1调用Nginx的时候设置了Keepalived，所以采用的是长连接，就没有这个端口号耗尽的限制。 ![Test](https://oscimg.oschina.net/oscnet/up-cb1706f8f544254d756b32eee5c1aa35c91.png  '解Bug之路-Nginx 502 Bad Gateway ') 
```
#### 1.9 Nginx 后面有多台机器的话
```
...
```  
#### 1.11 开启tcp_tw_reuse
这个问题Linux其实早就有了解决方案，那就是tcp_tw_reuse这个参数。 `echo '1' > /proc/sys/net/ipv4/tcp_tw_reuse` 事实上TIME_WAIT过多的原因是其回收时间竟然需要1min，这个1min其实是TCP协议中规定的2MSL时间，而Linux中就固定为1min。 
...
```
  ...
  `而其中的核心函数就是twsk_unique，它的判断逻辑如下:```java
```
  int tcp_twsk_unique(......)
```
{
...
#### 1.12 不要开启tcp_tw_recycle
开启tcp_tw_recyle这个参数会在NAT环境下造成很大的影响，建议不开启，具体见另一篇博客： 
 `my.oschina.net/alchemystar/blog/3119992`  
### 二、Nginx upstream改成长连接
...
```
### 四、总结
应用再强大也还是承载在内核之上，始终逃不出Linux内核的樊笼。所以对于Linux内核本身参数的调优还是非常有意义的。如果读过一些内核源码，无疑对排查线上问题有很大帮助，同时也能指导避过一些坑！ 
