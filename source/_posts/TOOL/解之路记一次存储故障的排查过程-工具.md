...
而笔者中间件这边收到的确是: 
 ```text 
  ...
  ```  
这的警。整个Bug现场如下图所示: ![Test](https://oscimg.oschina.net/oscnet/up-79247a4836df3b91fccf2ca49c46917a4c3.png  '解Bug之路-记一次存储故障的排查过程') 
#### 偶发性错误 
...
#### 只能靠日志分析了 
既然找不到什么头绪，那么只能从日志入手，好好分析推理了。REACTOR线程和Worker线程同时报错，但两者并无特殊的关联，说明可是同一个原因引起的两种不同现象。笔者在线上报错日志里面进行细细搜索，发现在大量的 
 ```text 
