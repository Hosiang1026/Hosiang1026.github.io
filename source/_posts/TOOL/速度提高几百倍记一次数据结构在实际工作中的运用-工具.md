...
##### 1. 这可能是个伪需求 
上面这个鞋子有11种颜色，11种尺码，意味着这些选项后面对应的是 ```text 
  11 * 11
  ``` ，总共 ```text 
  121
  ``` 个商品。如果再来个第三层选项，假设第三层也有 ```text 
  11
  ``` 个选项，那对应的商品总共就是 ```text 
  11 * 11 * 11
  ``` ，也就是 ```text 
  1331
  ``` 个商品，好多店铺总共可能都没有 ```text 
  1331
...
上面这个结构本身还是挺清晰的， ```text 
  ...
  ``` 是一个数组，有几层选项，这个数组就有几个对象，每个对象的 ```text 
  name
  ``` 就是当前层级的名字， ```text 
  values
  ``` 就是当前层级包含的选项，所以 ```text 
...
上面图片中，用户选择了第一层的 ```text 
  限量版574白粉
  ``` ，第二层的 ```text 
  40
  ``` ， ```text 
  41
  ``` 等不存在的商品就自动灰掉了。用上面的数据结构可以做到这个功能，当用户选择 ```text 
  限量版574白粉
  ``` 的时候，我们就去遍历 ```text 
  merchandise.products
  ``` 这个数组，这个数组的一个项就是一个商品，这个商品上的 ```text 
  variationMappings
  ``` 会有当前商品的 ```text 
  颜色
  ``` 和 ```text 
  尺码
  ``` 信息。对于我当前的项目来说，如果这个商品可以卖，他就会在 ```text 
  merchandise.products
  ``` 这个数组里面，如果不可以卖，这个数组里面压根就不会有这个商品。比如上图的 ```text 
  限量版574白粉
  ``` ， ```text 
  40
  ``` 码的组合就不会出现在 ```text 
...
所以对于 ```text 
  ...
  ``` ， ```text 
  40
  ``` 这个鞋子来说，为了知道它需不需要灰掉，我需要整个遍历 ```text 
  merchandise.products
  ``` 这个数组。按照前面说的 ```text 
  11
  ``` 个颜色， ```text 
  ...
  ``` ， ```text 
  41
  ``` 这个商品可以不可以卖，又要整个遍历商品数组，11个尺码就需要将商品数组整个遍历11次。对于两层选项来说， ```text 
  11 * 11
  ``` 已经算比较多的了，每个尺码百来次运算可能还不会有严重的性能问题。但是如果再加一层选项，新加这层假如也有 ```text 
  11
  ``` 个可选项，这复杂度瞬间就增加了一个指数，从 变成 ！现在我们的商品总数是 ```text 
  11 * 11 * 11
  ``` ，也就是 ```text 
  1331
  ``` 个商品，假如第三层是 ```text 
...
我拿着上述对求的疑问，和对性能的担心找到了产品经理，发生了如下对话： 
 
咱也不认识几个外国人，咱也不敢再问，都说了是用户需求，咱必须满足了产品才卖的出去，产品卖出去了咱才有饭吃，想办法解决吧！ 
 
#### 解决方案 
看来这个需求是必须要做了，这个功能并不复杂，因为三层选项可以沿用两层选项的方案，继续去遍历商品数组，但是这个复杂度增长是指数级的，即从 变成 ，用户用起来会卡。现在，我需要思考一下，有没有其他方案可以提高性能。经过仔细思考，我发现，这种指数级的复杂度增长是来自于我们整个数组的遍历，如果我能够找到一个方法不去遍历这个数组，立即就能找到 ```text 
  限量版574白粉
  ``` ， ```text 
  40
  ``` ， ```text 
...
##### 两层查找树 
为了说明白这个算法，我先简化这个问题，假设我们现在有两层选项， ```text 
  颜色
  ``` 和 ```text 
  尺码
  ``` ，每层选项有两个可选项： 
...
上面这颗树，我们忽略 ```text 
  root
  ``` 节点，在本例中他并没有什么用，仅仅是一个树的入口，这棵树的第一层淡黄色节点是我们第一层选项 ```text 
  颜色
  ``` ，第二层淡蓝色节点是我们的第二层选项 ```text 
  尺码
  ``` ，只是每个 ```text 
  颜色
  ``` 节点都会对应所有的 ```text 
  尺码
  ``` ，这样我们最后第二层的叶子节点其实就对应了具体的商品。现在我们要查找 ```text 
  红色
  ``` 的 ```text 
  39码
...
##### 三层查找树 
理解了上面的两层查找树，要将它扩展到三层就简单了，直接再加一层就行了。假如我们现在第三层选项是性别，有两个可选项 ```text 
  男
  ``` 和 ```text 
  女
  ``` ，那我们的查找树就是这样子的： 
...
##### 写代码 
上面算法都弄明白了，剩下的就是写代码了，我们主要需要写的代码就是用API返回的数据构建一个上面的 ```text 
  tree
...
##### 这就好了吗？ 
现在我们有了一颗查找树，当用户选择 ```text 
  红色
  ``` ， ```text 
  40
  ``` 码后，为了知道对应的 ```text 
  男
  ``` 可不可以点，我们不需要去遍历所有的商品了，而是可以直接从这个结构上取值。但是这就大功告成了吗？并没有！再仔细看下我们构建出来的数据结构，层级关系是固定的，第一层是颜色，第二层是尺码，第三层是性别，而对应的商品是放在第三层性别上的。也就是说使用这个结构，用户必须严格按照，先选颜色，再选尺码，然后我们看看性别这里哪个该灰掉。如果他不按照这个顺序，比如他先选了性别 ```text 
  男
  ``` ，然后选尺码 ```text 
  40
  ``` ，这时候我们应该计算最后一个层级 ```text 
  颜色
  ``` 哪些该灰掉。但是使用上面这个结构我们是算不出来的，因为我们并没有 ```text 
  tree["性别：男"]["尺码：40"]
  ``` 这个对象。 
这怎么办呢？我们没有 ```text 
  性别-尺码-颜色
  ``` 这种顺序的树，那我们就建一颗呗！这当然是个方法，但是用户还可能有其他的操作顺序呀，如果我们要覆盖用户所有可能的操作顺序，总共需要多少树呢？这其实是 ```text 
  性别
  ``` ， ```text 
  尺码
  ``` ， ```text 
  颜色
  ``` 这三个变量的一个全排列，也就是 ，总共 ```text 
  6
  ``` 颗树。像我这样的懒人，让我建6棵树，我实在懒得干。如果不建这么多树，需求又覆盖不了，怎么办呢，有没有偷懒的办法呢？如果我能在需求上动点手脚，是不是可以规避这个问题？带着这个思路，我想到了两点： 
 
###### 1. 给一个默认值。 
用户打开商品详情页的时候，默认选中第一个可售商品。这样就相当于我们一开始就帮用户按照 ```text 
  颜色-尺码-性别
  ``` 这个顺序选中了一个值，给了他一个默认的操作顺序。 
 
###### 2. 不提供取消功能，只能切换选项 
如果提供取消功能，他将我们提供的 ```text 
  颜色-尺码-性别
  ``` 默认选项取消掉，又可以选成 ```text 
  性别-尺码-颜色
  ``` 了。不提供取消功能，只能通过选择其他选项来切换，只能从 ```text 
  红色
  ``` 换成 ```text 
  白色
  ``` ，而不能取消 ```text 
  红色
  ``` ，其他的一样。这样我们就能永远保证 ```text 
  颜色-尺码-性别
...
欢迎![Test](https://oscimg.oschina.net/oscnet/86d70cfe-8b31-4ea4-a76f-8684f0a0a28c.png  '速度提高几百倍，记一次数据结构在实际工作中的运用')
