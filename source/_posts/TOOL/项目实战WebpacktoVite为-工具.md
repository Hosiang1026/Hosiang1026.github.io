...
我们项目也有启动慢的问题，同事也提到过几次。 刚好我之前也做过类似的探索和优化， 于是就借这个机会，改造一下项目，  
 ...
 。 
```
于昨天下午(2021.4.7 23:00)， 成功嵌入 Vite, 项目启动时间由约 `190s => 20s` , 热更新时间缩短为  
```
 `2s` 
 。 
...
根元素未找到。 
原因是： 默认生成的 index.html 中： `<div id="root"></div>` id 是 root, 而逻辑中的是 
 `#app` 
 , 这里直接改成 `id=app` 即可。 
...
没办法， 只得祭出  
 `window` 
  大法。 
在入口index.tsx 里面加上： `(window as any).global = window;` 刷新， 好了。 
...
谢谢。 
```
最后，如果觉得内容有帮助， 可以！[Test](https://oscimg.oschina.net/oscnet/up-de1450f6c8da91102bc534fdfd2fabce.png '-项目实战- Webpack to Vite， 为开发提速！')