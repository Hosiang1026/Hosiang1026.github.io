---
title: 飞桨与PyQt的碰撞，图形化目标检测So Easy
categories: 其他系列
tags:
  - Python
abbrlink: 36ee3c63
...
还记得3月份的时候我给大家介绍了PaddleDetection的环境部署、训练及可视化、模型导出。但那只是一个算法程序，一个完整的项目需要在算法的基础上将可视化操作开发成型。今天我给大家带来如何。..
<!-- more -->
...
2. 创建多线程检测机制分别给两个Button设置不同的槽函数，分别用于触发相机拍照和调用检测函数。运行infe_video.py可得到如下结果。由于作者使用的是黑白相机拍摄电脑屏幕上百度搜索出来的橘子照片，检测效果质量不高。 
